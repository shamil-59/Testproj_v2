...
async def list_pictures():
    return HTMLResponse(f"""<html><body>
        ...
        <hr><form action="/pictures/upload/" method="POST" enctype="multipart/form-data">
...
async def get_picture(item: str):
    try:
        ...
        async with aiofiles.open(path_to_file, "br") as pct:
            cont = await pct.read()
        return Response(content=cont, ) #media_type="image/png"
    except Exception:
        return {
            "error": 404,
...
async def upload_picture(file: UploadFile = File(...)):
    ...
    return HTMLResponse(        
        ...
        <hr><form action="/pictures"><button>BBACK TO STORAGE</button></form>
        """
        )
